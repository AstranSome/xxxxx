...
import { drinksMedia, type DrinkMedia } from "@/data/drinks";
import { drinksItems, drinksMenuPages } from "@/data/menu";
...
export function DrinksExperience() {
  const [activeItem, setActiveItem] = useState<DrinkMedia | null>(null);
  ...
  return (
    ...
    >
      <div>
        ...
        >
          Tap a pour below — iced coffees, espresso classics, and social column
          mocktails in motion.
        </motion.p>
      </div>
      ...
      <div className="space-y-8">
        {drinksMenuPages.map((page, pageIndex) => (
          <motion.div
            key={page.id}
            initial={{ opacity: 0, y: 24 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.45 + pageIndex * 0.1, duration: 0.5 }}
      ...
      <AnimatePresence>
        {activeItem && (
          <DrinkLightbox item={activeItem} onClose={() => setActiveItem(null)} />
        )}
      </AnimatePresence>
    </motion.div>
  );
